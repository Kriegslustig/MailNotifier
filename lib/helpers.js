...
module.exports = {
  ...
  getConfig: function () {
    var config = (this.readFile(this.getConfigFilePath('config.json')) || error('No config file found'))
  ...
  readJSONFile: function (file, fallback) {
    return JSON.parse(this.readFile(this.getFilePath('data', file)) || fallback)
  },
  ...
  formatWeekDay: function (weekDay) {
    ...
    console.log([this.firstIndex(days, (new Date()).getDay()), (new Date()).getDay()])
    ...
    console.log(((diff < 1 ? diff + days.length : diff) + adder))
